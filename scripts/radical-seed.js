...
const SCHEDULE_HOURS = [8, 10, 13, 15, 17, 19, 21, 23]; // Europe & US Slots
...
async function seed() {
    console.log('🚀 INITIALIZING GLOBAL BRIEF CONTENT ENGINE');
    console.log(`🎯 Target: ${MAX_ARTICLES} High-Quality Scheduled Articles`);
    ...
    async function generateContentWithRetry(prompt) {
        for (let i = 0; i < 3; i++) {
            try {
                const model = models[currentKeyIndex % models.length];
                const result = await model.generateContent(prompt);
                currentKeyIndex++;
                return result.response.text();
            } catch (e) {
                console.warn(`    ⚠️ Gen Error (Attempt ${i + 1}): ${e.message}`);
                currentKeyIndex++;
                await sleep(2000);
            }
        }
    ...
    // Calculate Schedule Base (Tomorrow)
    const scheduleBaseDate = new Date();
    scheduleBaseDate.setDate(scheduleBaseDate.getDate() + 1); // Tomorrow
    console.log(`📅 Scheduling for: ${scheduleBaseDate.toDateString()}`);
    ...
    for (const [category, url] of Object.entries(FEEDS)) {
        ...
        for (const match of items) {
            ...
            // --- AI GENERATION ---
            const prompt = `You are an elite journalist. Rewrite this headline: "${title}" into a premium news article.
            STRICT RULES:
            - Length: 800+ words.
            - Format: Markdown (Use ## Headers, **Bold**, > Quotes).
            - Tone: Objective, Expert, Global News Style.
            ...
            OUTPUT JSON ONLY:
            {
                ...
                "metaDescription": "SEO summary (150 chars)",
                "keywords": ["tag1", "tag2"]
            }`;
            ...
            console.log(`    ✅ SAVED & SCHEDULED for ${scheduledTime.toLocaleTimeString()} (Tomorrow)`);
            totalGenerated++;
